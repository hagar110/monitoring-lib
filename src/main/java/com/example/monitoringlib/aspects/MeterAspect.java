...
public class MeterAspect {
    ...
    private void processAnnotations(List<Meter> annotations, ProceedingJoinPoint joinPoint, Method method) throws Throwable {
        ...
        Object outcome = null;
        try {
            outcome = joinPoint.proceed();
        } catch (Exception ex) {
            annotations.stream().filter(annotation -> annotation.recordingTime() == RecordingTime.ERROR).forEach(e -> recordMeter((Meter) e, argsMap, ex, Long.MAX_VALUE));
        }
        if (outcome != null) {
            long durationInNanos = System.nanoTime() - startInNanos;
            Object finalOutcome = outcome;
            annotations.stream().filter(annotation -> annotation.recordingTime() == RecordingTime.AFTER).forEach(e -> recordMeter((Meter) e, argsMap, finalOutcome, durationInNanos));
        }
    }
